...
WARNING: Never use _dev mode_ mode in production as it is unsecured by default.
...
This could be limited by setting a more restricted expression in `path` section.
...
Finally, connect the kubernetes auth method with the previous created policy and the service account created when our service is deployed (seen later).
...
. `myapprole` is a role for the service autoamtically logged to using this mechanism. Our service must be aware of it, and you need to set it on Vault client (seen later).
...
Now, it is time to deploy an application that authenticate against Vault automatically using the Kubernetes auth mode, and take the secret that you created in the previous section from the Vault service.
...
All the code is avalable at `$TUTORIAL_HOME/services/vault-started`.
...
<3> Role created when configuring the Kubernetes auth.
...
When the Quarkus application is started, Vault extension authenticates using the Kubernetes auth method, and then it queries Vault server to get the required secrets and inject them in the resource.

[#transit]
== Transit

Transit mode can be used from your application to encrypt and decrypt data.
You can think this as a cryptography as a service.
...
Quarkus application deployed in the previous section, already implemets the logic to encrypt and decrypt in transit, so you don't need to deploy anything else new.
...
It is not necessary to udeploy anything, as the same services are needed in the next section.
