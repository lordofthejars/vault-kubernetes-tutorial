...
----
minikube ssh -p=secrets-vault "$(cat ./scripts/install_vault_kms.sh)" -p vault
----
...
Injecting the secret as environment variable or as a volume has explained in xref:k8s_secrets.adoc#usesecrets[Using Secrets] section.
...
* Secret is injected inside the pod, anyone with access to the pod can find the mounted volumes hence print the secrets.
